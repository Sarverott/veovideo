...
const modeIcons: Record<GenerationMode, React.ReactNode> = {
  ...
  [GenerationMode.STORYBOARD]: <StoryboardIcon className="w-5 h-5" />,
};
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const promptPlaceholder = {
    ...
    [GenerationMode.STORYBOARD]:
      'Paste your entire story or script here. AI will split it into scenes for you...',
  }[generationMode];
  ...
  const renderMediaUploads = () => {
    ...
    // References available for REFERENCES_TO_VIDEO and STORYBOARD
    if (
      generationMode === GenerationMode.REFERENCES_TO_VIDEO ||
      generationMode === GenerationMode.STORYBOARD
    ) {
      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col gap-2">
          {generationMode === GenerationMode.STORYBOARD && (
             <p className="text-xs text-gray-400 text-center w-full mb-1">
               Add reference images for characters (e.g. Cat, Eliot) to maintain consistency across scenes.
             </p>
          )}
  ...
  const isStoryboardMode = generationMode === GenerationMode.STORYBOARD;
  ...
  switch (generationMode) {
    ...
    case GenerationMode.STORYBOARD:
  ...
  return (
    <div className="relative w-full">
      {isSettingsOpen && !isStoryboardMode && (
