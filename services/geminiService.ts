...
export const createStoryboard = async (scenario: string): Promise<string[]> => {
  const ai = new GoogleGenAI({apiKey: process.env.API_KEY});

  // Using Flash for fast text logic reasoning
  const response = await ai.models.generateContent({
    model: 'gemini-2.5-flash',
    contents: scenario,
    config: {
      systemInstruction:
        'You are an expert video director. Your goal is to take a long, complex scenario description and split it into a chronological list of distinct, visually descriptive prompts. Each prompt must be suitable for a video generation AI (like Veo), meaning it should describe a single continuous shot of about 5-8 seconds. Focus on visual details, camera angles, and action. Return ONLY a JSON array of strings.',
...
export const generateVideo = async (
  ...
  // 2. Handle Start/End Frames (Image Input)
  // This applies if specific frames are provided, regardless of the "Mode" label
  ...
  // Handle End Frame (looping or distinct)
  ...
  if (referenceImagesPayload.length > 0) {
    ...
    // Force model upgrade if not already set, as references require the stronger model
    if (generateVideoPayload.model === VeoModel.VEO_FAST) {
       console.log("Upgrading model to Veo Generate Preview to support reference images.");
       generateVideoPayload.model = VeoModel.VEO;
    }
  }

  // 4. Handle Video Input (Extension)
  if (params.mode === GenerationMode.EXTEND_VIDEO) {
